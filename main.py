...
class MainWindow(QMainWindow):
    ...
    def on_finished(self, code: int) -> None:
        ...
        if stopped or code != 0:
            return
        # 跑完自动刷新页面
        if getattr(self, "last_module", "") == "analysis.live_view":
...
def run_task(module: str, extra: list[str] | None = None) -> int:
    ...
    等价于 `python -m analysis.report`，只是不需要外部 Python 环境。
    extra 是传给那个模块的参数，比如 `--task run_daily bilibili` 里的 bilibili。
    """
    import runpy
    sys.argv = [module, *(extra or [])]     # ★ 必须改：否则被跑的模块会看到 --task 这些参数，
    runpy.run_module(module, run_name="__main__")   #   像 run_daily 会把 --task 当成"站点名"
    return 0
